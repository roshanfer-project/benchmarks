...
type Server struct {
	pb.UnimplementedMS_2687Server
}

const serviceName = "MS_2687"
...
func (s *Server) Run() error {
	...
	pb.RegisterMS_2687Server(srv, s)

	reflection.Register(srv)
	port := 2000
	if meshProxy {
		port = utils.StrToInt(utils.GetEnvVar("MS_2687_PORT", true))
	}
...
func (s *Server) VdboDuPbKj(ctx context.Context, req *pb.Request) (*pb.Response, error) {
	benchExpBusyLoop(1.5)
