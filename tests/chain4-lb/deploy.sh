...
if [ "$MODE" = "sidecar" ]; then
  ...
  kubectl create configmap chain4-lb-config --from-env-file="$TMP_DIR/sidecar_merged.env" --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  ...
  for SVC in backend3 backend2 backend1 frontend; do
  ...
  kubectl create configmap chain4-lb-config --from-env-file="$TMP_DIR/sidecar_lb_merged.env" --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
  ...
  kubectl create configmap chain4-lb-config --from-env-file="$TMP_DIR/envoy_merged.env" --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
  ...
  kubectl create configmap chain4-lb-config --from-env-file="$TMP_DIR/rajomon_merged.env" --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
  ...
  for IMG in backend1 backend2 backend3 frontend-grpc rajomon-client; do
    sed -i "s|${BENCH}-${IMG}:latest|${REGISTRY}/${BENCH}-${IMG}:${TAG}|g" "$TMP_DIR/app-grpc.yaml"
  done
  deploy_fail=0
  declare -a deploy_pids=()
  for SVC in backend3 backend2 backend1 frontend-grpc rajomon-client; do
    (
  ...
  kubectl create configmap chain4-lb-config --from-env-file="$TMP_DIR/dagor_merged.env" --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
  ...
  kubectl create configmap chain4-lb-config --from-env-file="$TMP_DIR/dagor_lb_merged.env" --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
  ...
  kubectl create configmap chain4-lb-config --from-env-file="$TMP_DIR/rajomon_lb_merged.env" --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
  ...
  kubectl create configmap chain4-lb-config --from-env-file=k8s/plain-lb.env --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
  ...
  kubectl create configmap chain4-lb-config --from-env-file=k8s/plain.env --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
