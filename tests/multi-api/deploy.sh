...
if [ "$MODE" = "roshanfer" ]; then
  ...
  kubectl create configmap multi-api-config --from-env-file="$TMP_DIR/amphiqueue_merged.env" --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
  ...
  kubectl create configmap multi-api-config --from-env-file=k8s/${MODE}.env --dry-run=client -o yaml > "$TMP_DIR/configmap.yaml"
  kubectl apply -f "$TMP_DIR/configmap.yaml"
