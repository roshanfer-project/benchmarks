...
fail=0
declare -a pids=()
for kn in "backend1" "backend2" "backend3" "frontend"; do
  (
...
kubectl delete configmap fan-out-3-config --ignore-not-found
...
if [ "$MODE" = "rajomon" ] || [ "$MODE" = "dagor" ] || [ "$MODE" = "dagor-lb" ] || [ "$MODE" = "rajomon-lb" ]; then
  ...
  kubectl delete service fan-out-3-entry --ignore-not-found
