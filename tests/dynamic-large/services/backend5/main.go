...
import (
	...
	"fmt"
	"net"
<<<<<<< HEAD:tests/prob-route-fanout/services/backend5/main.go
	"probroutefanout/pkg"
...
=======
	...
	"dynamiclarge/utils"
>>>>>>> 7e20238 (rename test benchmark):tests/dynamic-large/services/backend5/main.go
...
type Server struct {
	pb.UnimplementedBackend5Server
}

const serviceName = "backend5"
...
func (s *Server) Run() error {
	...
	pb.RegisterBackend5Server(srv, s)

	reflection.Register(srv)
	port := 2000
	if sidecar {
		port = utils.StrToInt(utils.GetEnvVar("backend5_PORT", true))
	}
...
func (s *Server) F6(ctx context.Context, req *pb.Request) (*pb.Response, error) {
	utils.BusyLoop(960)
