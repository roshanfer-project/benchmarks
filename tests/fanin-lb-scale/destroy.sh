...
fail=0
declare -a pids=()
for kn in "backend1" "backend2" "frontend" "shared"; do
  (
...
kubectl delete configmap fanin-lb-scale-config --ignore-not-found
...
if [ "$MODE" = "rajomon" ] || [ "$MODE" = "dagor" ] || [ "$MODE" = "dagor-lb" ] || [ "$MODE" = "rajomon-lb" ]; then
  ...
  kubectl delete service fanin-lb-scale-entry --ignore-not-found
