...
cd "$ROOT_DIR"
kubectl delete pod -l app=backend --ignore-not-found --wait=true
kubectl delete service -l app=backend --ignore-not-found
kubectl delete pod -l app=frontend --ignore-not-found --wait=true
kubectl delete service -l app=frontend --ignore-not-found
kubectl delete configmap chain-2-bimodal-config --ignore-not-found
...
if [ "$MODE" = "rajomon" ] || [ "$MODE" = "dagor" ]; then
  ...
  kubectl delete service chain-2-bimodal-entry --ignore-not-found
