...
mkdir -p "$OUTPUT_DIR"
declare -a log_pids=()
for svc in backend1 backend2 frontend shared frontend-grpc rajomon-client; do
...
if [ "$MODE" = "envoy" ]; then
  ...
  for svc in backend1 backend2 frontend shared frontend-grpc rajomon-client ingress; do
