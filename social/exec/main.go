...
func main() {
	...
	// remove /tmp/SOCIAL.ready if it exists
	if _, err := os.Stat("/tmp/SOCIAL.ready"); err == nil {
		os.Remove("/tmp/SOCIAL.ready")
	}
	...
	if err != nil {
		fmt.Println("Error creating /tmp/SOCIAL.ready:", err)
		cancel()
		panic(err)
	}
