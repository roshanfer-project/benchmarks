...
func (s *Server) Run() error {
	...
	conn := hotel.GetConn(utils.GetEnvVar("SearchAddr", true), options...)
	s.searchClient = search.NewSearchClient(conn)

	conn = hotel.GetConn(utils.GetEnvVar("ProfileAddr", true), options...)
	s.profileClient = pb.NewProfileClient(conn)

	conn = hotel.GetConn(utils.GetEnvVar("UserAddr", true), options...)
	s.userClient = user.NewUserClient(conn)

	conn = hotel.GetConn(utils.GetEnvVar("ReservationAddr", true), options...)
