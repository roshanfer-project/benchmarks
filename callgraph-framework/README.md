...
- `load_balancing_policy` (optional, default `round_robin`, **plain-lb, dagor-lb, and rajomon-lb**): gRPC client load balancer — `round_robin`, `least_request` (P2C over active RPC counts), or `weighted_round_robin` (WRR enables ORCA server metrics).
