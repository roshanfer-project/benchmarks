...
func (s *Server) Run() error {
	...
	if sidecar {
		conn = pkg.DialClient(utils.GetEnvVar("MS_62039_EGRESS", true), sidecar)
	}
	if !sidecar {
		addr := utils.GetEnvVar("MS_18750_ADDR", true)
		if useRajomon {
	...
	port := 2000
	if sidecar {
		port = utils.StrToInt(utils.GetEnvVar("MS_62039_PORT", true))
	}
